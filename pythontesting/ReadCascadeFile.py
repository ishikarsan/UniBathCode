...
class Circuit:
    ...
    def __init__(self, node1, node2, resistorValue, capacitanceValue,
                 inductanceValue, conductorValue, frequency):

        """
        function establishes objects which are used in the imepedence calculations;
        sends either B or C to be calculated depending on the whether the component
        is in series ie. n2 =/= 0 or parallel where n2 = 0
...
def ReadCascadeFile(inputFile):
    ...
    returnDict = {}
    returnDict.update({"cascadeFile":cascadeFile,
...
def getTerms(termBlock):
    ...
    """
    CascadeValue = {"VT":None, "RS":None, "RL":None, "IN":None, "GS":None,
                    "Fstart":None, "Fend":None, "Nfreqs":None, "LFstart":None, "LFend":None}
    CascadeParameters = [
        {
            "name":"VT",
            "type":"FLOAT"
    ...
    for termLine in termBlock:
        for termParam in CascadeParameters:
            aval, err = getValueFromString(termParam.get("name"),
                                           termParam.get("type").lower(),
                                           termLine)
            if err == None:
                CascadeValue.update({termParam.get("name"):aval})
            
    if ((CascadeValue["Fend"] == None and CascadeValue["LFend"] == None) or 
        (CascadeValue["Fstart"] == None and CascadeValue["LFstart"] == None) or 
        (CascadeValue["Nfreqs"] == None)):
        print ("Missing Terms")
...
def getOutputBlock(cBlock):
    ...
    for a in cBlock:
        ...
        for item in getOutputDetails(pos, scale, units, 
                                     dbCalc, outputTerms[0], label1, None):
...
def getOutputDetails(pos, scale, units, dbCalc, name, label1, label2):
    ...
    if len(units) == 0:
        units = "L"
    outputDict.update({"position":pos, "name":name, 
                       "units":units, "scale":scale, 
                       "dbCalc":dbCalc, "label":label1})
    extended.append(outputDict.copy())

    if label2 != None:
        outputDict.update({"position":pos+1, "name":name, 
                           "units":units, "scale":scale, 
                           "dbCalc":dbCalc, "label":label2})
        extended.append(outputDict.copy())
...
def getCascadeCalc(freq, theTerms, abcd):
    ...
    Iout = Vout / RL
    Iin = Iout / Ai
    Pin = Vin * numpy.conj(Iin)
    Pout = Vout * numpy.conj(Iout)
    ...
    dBPout = 20*math.log(abs(Pout), 10)
...
def processCascadeFile(cascadeFile):
    ...
    if ablock == None:
        reportError("Error reading file", "ablock")
        return
    if cblock == None:
        reportError("Error reading file", "cblock")
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', required=False, default=None, metavar='a_Test_Circuit_1', 
                        help='Input file name should be without the .net extension')      
    parser.add_argument('-p', required=False, default=None, metavar='[1,2,3]', 
                        help='Optional list of output columns e.g. [2,4,6]')
